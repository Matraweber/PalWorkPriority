...
-- Restored from fb9be28, the second of the five changes rolled back on
-- 21 August, brought back alone now that mod_class has been proven alone.
-- Building
-- ---------------------------------------------------------------------------
...
function M.host()
    ...
    -- check for it.
    widget, tree, canvas = nil, nil, nil
    -- The parts go with the widget they came out of. A flag that describes an
    -- engine object has to share its lifetime, and these are handles into a
    -- tree that was just let go of.
    M.parts = nil

    -- The blueprint if the pak mounted, our own canvas if it did not. The
    -- fallback is the entire point: that canvas is what has drawn this panel
    -- all along, so a pak that fails to mount costs the new look rather than
    -- the whole menu.
